...
test.describe('Chat panel without model', () => {
  ...
  test('should open the chat panel', async ({ page }) => {
    const chatIcon = page.getByTitle('Jupyterlite AI Chat');
    await chatIcon.click();
    await expect(page.locator('[id="@jupyterlite/ai:chat-widget"]')).toBeVisible();
  });
  ...
  test('should receive an error message', async ({ page}) => {
    ...
    await expect(messages.first().locator('.jp-chat-rendered-markdown')).toHaveText(content);

    await expect(messages.last().locator('.jp-chat-message-header')).toHaveText(/^ERROR/);
    await expect(messages.last().locator('.jp-chat-rendered-markdown')).toHaveText('AI provider not configured');
  });
})

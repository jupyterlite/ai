...
export class WebLLMCompleter implements IBaseCompleter {
  constructor(options: BaseCompleter.IOptions) {
    this._webllmProvider = new ChatWebLLM({
      model: 'Phi-3-mini-4k-instruct-q4f16_1-MLC',
      chatOptions: {
        temperature: 0.5
      }
    });
    void this._webllmProvider.initialize(progress => {
  ...
  async fetch(
    ...
    try {
      ...
      const response = await this._webllmProvider.invoke(messages);
  ...
  private _webllmProvider: ChatWebLLM;
